...
export class CalculadoraComponent implements OnInit {
  ...
  concatenarNumero(numAtual: string, numConcat: string): string {
    ...
    //primeiro dígito é '.', contacatena '0' antes do ponto
  ...
  /**
   * Executa alógica quando um operador for selecionado
   * Caso já possua uma operação selecionada, execita a operação anterior, e define a nova operação.
   * 
  ...
  definirOperacao(operacao: string): void {
    ...
    if(this.numero2 !== null) {
      this.resultado = this.calculadoraService.calcular(parseFloat(this.numero1), parseFloat(this.numero2),
      this.operacao)
      this.operacao = operacao
